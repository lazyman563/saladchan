...
// ── GET /api/threads?board=x&sort=new|hot|replies&page=0&limit=20 ──────────
export async function GET(req: NextRequest) {
  ...
  let query = supabase
    ...
    .select(`
      *,
      author:profiles!author_id (id, username, display_name, avatar_url, role, verified),
      board:boards!board_id (id, name, icon, color)
    `)
  ...
  if (user && threads?.length) {
    ...
    likedIds = (likes ?? []).map(l => l.thread_id!)
  }

  const enriched = (threads ?? []).map(t => ({ ...t, liked_by_me: likedIds.includes(t.id) }))
...
export async function POST(req: NextRequest) {
  ...
  if (ban) return NextResponse.json({ error: 'Sua conta está banida.' }, { status: 403 })
...
// ── PATCH /api/threads — pin/lock/delete (mod actions) ───────────────────
export async function PATCH(req: NextRequest) {
  ...
  // Verify mod/admin for mod actions
  if (['pin','lock','delete_mod'].includes(action) && !['mod','admin'].includes(prof?.role ?? ''))
